...
root = tk.Tk()
root.title("Andromeda 2.3 - Parâmetros de Operação")
...
def obter_parametros():
    global AtivoAoperar, Contratos, mediaMovelRapida, mediaMovelLenta, StopGain, StopLoss, AcionarNovoStop, novo_stop_gain, novo_stop_loss, Atualizar
...
def estrategia_trade(dados, ativo):
    ...
    atualizar_texto(f"Robo Andromeda 2.0 - Compra e Venda. Hora atual: {hora_agora}")
...
def iniciar_estrategia():
    ...
    while estrategia_ativa:
        ...
        if hora_atual >= 9 and hora_atual < 18:  # Horário de operação (10h às 18h)
            ...
            data_inicial = datetime.today() - timedelta(days=5)
